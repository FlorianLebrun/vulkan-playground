...
function(append_static_file_copy LIST)
  ...
  foreach(filename ${ARG_FILES})
    set(inputFile "${ARG_FROM}/${filename}")
...
############################################
# Clickonce packaging functions
############################################

function(append_clickonce_manifest LIST)
  cmake_parse_arguments(ARG "" "APPNAME;TO" "DEPENDS" ${ARGN})
  set(outputFile 
    "${ARG_TO}/${ARG_APPNAME}.manifest"
    "${ARG_TO}/${ARG_APPNAME}.application"
  )
  add_custom_command(
    OUTPUT ${outputFile}
    DEPENDS ${ARG_DEPENDS}
    WORKING_DIRECTORY ${TOOLS_NODE_SCRIPT_DIR}
    COMMAND node make_clickonce_manifests
      --name ${ARG_APPNAME}
      --version ${CMAKE_PROJECT_VERSION}
      --destination ${ARG_TO}
  )
